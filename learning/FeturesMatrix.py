...
def build_matrix_from_fetures(vertex_to_tag,map_fetures):
    node_to_fetures = {}
    k = list(map_fetures.keys())[0]
    for n in list(vertex_to_tag.keys()):
        if n in list(map_fetures[k].keys()):
            node_to_fetures[n] = []
...
def build_matrix_with_tags(gnx, map_features, vertex_to_tag, zscoring = True):
    ...
    return [feature_matrix, tags_vector,node_to_features, nodeWithTags_to_features]
